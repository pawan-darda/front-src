...
var strDownloadMime = "application/x-download";
 ...
 */
 ...
 // Since we do not want to save the image object we are changing some things 
 
Dygraph.Export.asPNG = function (dygraph, userOptions) {
    "use strict";
    var options = {}, 
...
Dygraph.Export.putVerticalLabel = function (ctx, divLabel, options, font, color, textAlign) {
    ...
    if (textAlign == "center") {
        ...
        top = Math.ceil((500 - textDim.width) / 2);
    }
...
Dygraph.Export.drawLegend = function (canvas, dygraph, options) {
    ...
    for (i = 0; i < labels.length; i++) {
        if ( $.trim(labels[i]) == "x-axis") {continue}
